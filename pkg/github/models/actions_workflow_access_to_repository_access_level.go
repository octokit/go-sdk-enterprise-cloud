package models
// Defines the level of access that workflows outside of the repository have to actions and reusable workflows within therepository.`none` means the access is only possible from workflows in this repository. `user` level access allows sharing across user owned private repositories only. `organization` level access allows sharing across the organization. `enterprise` level access allows sharing across the enterprise.
...
const (
    ...
    ENTERPRISE_ACTIONSWORKFLOWACCESSTOREPOSITORY_ACCESS_LEVEL
)

func (i ActionsWorkflowAccessToRepository_access_level) String() string {
    return []string{"none", "user", "organization", "enterprise"}[i]
}
func ParseActionsWorkflowAccessToRepository_access_level(v string) (any, error) {
    ...
    switch v {
        ...
        case "enterprise":
            result = ENTERPRISE_ACTIONSWORKFLOWACCESSTOREPOSITORY_ACCESS_LEVEL
        default:
            return nil, nil
    }
    return &result, nil
